...
from ..models import Bin, AggregateDatasetMetric
...
class DatasetAggFiltersMixin:
    """
    custom mixin to handle all filtering by query_params for IFCB Dataset Aggregate data
    """
    ...
    def handle_query_param_filters(self, queryset):
        ...
        # create empty Q objects to handle conditional filtering
        date_q_filters = Q()
        # calculate whether to use monthly or daily aggregates based on time range
        # Use daily unless range is greater than 5 years
        difference_in_years = relativedelta(end_date_obj, start_date_obj).years
        timespan_type = AggregateDatasetMetric.DAILY
        if difference_in_years > 5:
            timespan_type = AggregateDatasetMetric.MONTHLY
        print(timespan_type)
        if start_date or end_date:
            # if "seaonsal" filter is True, need to get multiple date ranges across the time series
            if seasonal:
        ...
        queryset = queryset.prefetch_related(
            Prefetch(
                ...
                queryset=AggregateDatasetMetric.objects.filter(date_q_filters).filter(
