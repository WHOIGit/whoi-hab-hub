...
def get_hab_species_default():
    ...
    station_default = [
        species for species in all_species_list if species == "Alexandrium_catenella"
    ]
...
class Station(models.Model):
    ...
    hab_species = fields.ChoiceArrayField(
        models.CharField(
...
class Datapoint(models.Model):
    station = models.ForeignKey(
        Station, related_name="datapoints", on_delete=models.CASCADE, null=False
    )
    ...
    def __str__(self):
        return f"{self.station.station_location} - {self.measurement_date}"
